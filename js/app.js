...
function demoData() {
  ...
  const mk = (n, cid, status, issuedOff, dueOff, lines, tax = 8) => ({
  ...
  const invoices = [
    mk(1042, "c1", "paid", -150, -120, [L("Brand identity design", 1, 2400), L("Website build (12 hrs)", 12, 120)]),
    mk(1043, "c2", "paid", -118, -88, [L("Monthly retainer — design", 1, 1800)]),
    mk(1044, "c3", "paid", -85, -55, [L("Campaign landing pages", 3, 650)]),
    mk(1045, "c2", "paid", -60, -30, [L("Monthly retainer — design", 1, 1800)]),
    mk(1046, "c4", "paid", -45, -15, [L("Design system audit", 1, 1200), L("Component library (8 hrs)", 8, 110)]),
    mk(1047, "c1", "paid", -22, -2, [L("Marketing site refresh", 1, 3200)]),
  ...
  $("invBody").innerHTML = rows.map(({ inv, s, c }) => `
    <tr>
      ...
      <td><div class="row-actions">
        ...
        ${s === "sent" || s === "overdue" ? `<button class="icon-btn" title="Mark paid" onclick="setStatus('${inv.id}','paid')">💸</button>` : ""}
...
window.addEventListener("resize", () => {
  if (!$("appShell").hidden && !$("view-dashboard").hidden) drawRevChart();
});
